...
- **Multimodal support: audio/video transcription (Phase 1, first cut)** — new `graphify-media` crate plus a `Media` file type in detection (`.mp4`, `.mov`, `.avi`, `.mkv`, `.webm`, `.m4v`, `.mp3`, `.wav`, `.m4a`, `.flac`, `.ogg`, `.opus`). Media files are transcribed with a locally installed Whisper tool — discovery order: `GRAPHIFY_WHISPER_CMD` (custom stdout command) → `whisper-cli` (whisper.cpp + GGML model) → OpenAI `whisper` Python CLI — with no new linked dependencies. Each file yields a transcript concept node and a `transcribes` edge; with an LLM configured, transcripts also go through semantic extraction (file type `media`). Transcripts are cached by content hash in `<output>/cache/media/` and are reused even when no Whisper tool is installed. `yt-dlp` discovery is in place for upcoming URL audio ingestion.
